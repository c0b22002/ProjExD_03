...
class Bird:
    ...
    def __init__(self, num: int, xy: tuple[int, int]):
        ...
        """
        img_0 = pg.transform.rotozoom(pg.image.load(f"ex03/fig/{num}.png"), 0, 2.0)
        img = pg.transform.flip(img_0, True, False) #右向き
        ...
        self.imgs= {
            ...
            (+5, +5): pg.transform.rotozoom(img, -135, 1.0) #右下
        }
    ...
    def update(self, key_lst: list[bool], screen: pg.Surface):
        ...
        if not (sum_mv[0] == 0 and sum_mv[1] == 0): #任意のキーが押されたら
    ...
    while True:
        for event in pg.event.get():
            ...
            if event.type == pg.KEYDOWN and event.key == pg.K_SPACE:
                beam = Beam(bird) #ビームクラスのインスタンスの生成
